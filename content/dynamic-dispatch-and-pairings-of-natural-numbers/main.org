...
proof by cases that I think could lead to a solution for any =T= and
=S=.
...
=std::integral_constant= types that lift the values of =enum=, and the
return type is whatever that invocable returns.

Then an implementation of the FSM shown at the beginning of the
previous section would go as follows

#+include: ./code/basic.cpp src c++ :lines "210-274"

Perhaps it is not the prettiest, but it has the basic machinery to get
going. Aliases and what not can be used to prettify in the aftermath.

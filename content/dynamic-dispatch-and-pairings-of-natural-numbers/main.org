...
Pairing functions in math are often introduced as a way to demonstrate
that the sets $\mathbb{N}$ and $\mathbb{N} \times \mathbb{N}$ have the
same "size" (more formally, cardinality).  Quickly after this
explanation, it often follows that for any finite number of factors,
the set $\mathbb{N} \times ... \times \mathbb{N}$ has the same "size"
as the set $\mathbb{N}$, annotated as $\mathbb{N} \times ... \times
\mathbb{N} \cong \mathbb{N}$.


I will now present the proof of $\mathbb{N} \times ... \times
\mathbb{N} \cong \mathbb{N}$ when the number of factors is greater
than $2$, using some pairing function $e : \mathbb{N} \times
\mathbb{N} \to \mathbb{N}$.  We will mimick the shape of this proof to
write our single integer encoding of an arbitrary number of =enum=.
...
\mathbb{N}$, then we can construct a bijection $e' : \mathbb{N} \times
\mathbb{N} \times \mathbb{N} \to \mathbb{N}$ by defining $e'$ to be
$e'(x, y, z) := e(e(x, y), z)$. Since $e$ is invertible, if we denote
the projection of the first and second element of a pair by $\pi_0$
and $\pi_1$, respectively, we can define the inverse of $e'$ by

\[e'^{-1}(u) := (\pi_0(e^{-1}(\pi_0(e^{-1}(u)))), \pi_1(e^{-1}(\pi_0(e^{-1}(u)))), \pi_1(e^{-1}(u))).\]

This seems like a mouthful but its mainly because of the notational
noise intreduced by projections. The deconstruction is merely
mimicking the shape of $e'$. Notice

\begin{align*}
\pi_0(e^{-1}(u)) &= e(x, y),\\
\pi_1(e^{-1}(u)) &= z,\\
e^{-1}(\pi_0(e^{-1}(u))) &= (x, y),\\
\pi_0(e^{-1}(\pi_0(e^{-1}(u)))) &= x,\\
\pi_1(e^{-1}(\pi_0(e^{-1}(u)))) &= y\\
\end{align*}

Using a function $c$ that appends an element to an $n$ tuple, yielding
an $n+1$ tuple, then we can simplify to
...
Now for $4$ factors we want to construct a bijection $e'' : \mathbb{N}
\times \mathbb{N} \times \mathbb{N} \times \mathbb{N} \to \mathbb{N}$
from the pairing function $e$.  This could be defined as $e''(w, x, y,
z) := e(e'(w, x, y), z) = e(e(e(w, x), y), z)$.  At this point we can
already begin to see an [[https://wiki.haskell.org/index.php?title=Fold][accumulating or folding]] pattern emerging.

The inverse $e''^{-1}$ is just
...
Inspecting [[eq-1][Equation 1]] and [[eq-2][Equation 2]] we can notice that not only is
the structural similarity of $e''^{-1}$ with the definiton of $e''$ as
clear as that of $e'^{-1}$ with $e'$, but it is actually the same.  In
the definiton of both inverses the innermost usage of $e$ corresponds
to a destructuring of an encoding into the parts which it encodes, and
the outermost usage of the inverse with one less prime ($'$) is the
inductive step.

Thus we procede to write the variadic case as
...
\begin{equation}
 e^{n}(x) :=
   \begin{cases}
    x &\text{if } n = 1\\
    e(e^{n-1}(\pi_{0}(x), ..., \pi_{n-1}(x)), \pi_{n}(x)) &\text{otherwise}
  \end{cases}.
\end{equation}

...
previous section from the user side would look as below
...
1. (compile time) For each =enum= (i.e., =StateT=, =InputT=), list its
   values.

2. (compile time) For each =enum=, map its values to per =enum= unique
   =unsigned integral= values. This map must be a bijective function
   $g$ --- we will need to undo this mapping later with $g^{-1}$.

3. (compile time) Obtain the cartesian product of the lists of
   =unsigned integral= values.

4. (compile time) Map each element in the cartesian product to a
   single =unsigned integral= using =szudzek_pair=.

5. (compile time) Use the resulting list of =unsigned integral= to
   construct a flat =switch=.

6. (compile-time) Since the associated value to a =case= is constant,
   use =szudzek_unpair= and $g^{-1}$ to obtain compile time values
   $(x, y)$ for each possible =case= label and invoke the argument
   function =f= using =integral_constant= tags constructed from $x$
   and $y$.

7. (runtime) For the input =state= and =input= obtain its encoding $s$
   using =szudzek_pair=.
